...
req() { _req "$1" "$2" -H "User-Agent: $user_agent" --header="Content-Type: application/octet-stream" --header="Accept-Language: en-US,en;q=0.9" --header="Connection: keep-alive" --header="Upgrade-Insecure-Requests: 1" --header="Cache-Control: max-age=0" --header="Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8"; }
...
get_highest_ver() {
	...
	vers=$(tee)
	m=$(head -1 <<<"$vers")
	if ! semver_validate "$m"; then echo "$m"; else sort -rV <<<"$(printf '%s\n' "$vers" | awk '{b=$0; sub(/-.*/,"",b); print b "|" $0}')" | cut -d'|' -f2 | head -1; fi
}
semver_validate() {
	local a="${1%-*}"
	local a="${a#v}"
...
--keystore-entry-password=$KEYSTORE_PASS --keystore-password=$KEYSTORE_PASS --signer=$KEYSTORE_ALIAS --keystore-entry-alias=$KEYSTORE_ALIAS $patcher_args"
